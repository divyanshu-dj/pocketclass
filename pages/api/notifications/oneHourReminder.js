...
export default async function handler(req, res) {
  ...
  try {
    console.log('Starting 1-hour class reminder process...');
    ...
    // Filter bookings that need 1-hour reminders
    const bookingsNeedingReminders = bookings.filter((booking) => {
      ...
      const reminderTime = moment().tz(booking.timezone || "America/Toronto").add(1, "hour");
    ...
    console.log(`Found ${bookingsNeedingReminders.length} bookings needing 1-hour reminders`);
    ...
    for (const booking of bookingsNeedingReminders) {
      try {
        ...
        // Load email template
        const htmlContent = loadEmailTemplate('oneHourReminder.html', templateData);
        ...
        const emailPromise = sendEmailToBookingRecipients(
          ...
          if (result.success) {
            ...
            console.log(`1-hour reminder sent for booking ${booking.id} to ${result.emailsSent || 1} recipient(s)`);
          } else {
            console.error(`Failed to send 1-hour reminder for booking ${booking.id}:`, result.error);
          }
          return result;
        });
    ...
    console.log(`1-hour class reminder process completed. Sent ${emailsSent} emails.`);

    return res.status(200).json({
      success: true,
      message: `Sent ${emailsSent} one-hour class reminder emails`,
      emailsSent,
      totalBookingsProcessed: bookingsNeedingReminders.length
    });

  } catch (error) {
    console.error('Error in 1-hour class reminder handler:', error);
    return res.status(500).json({
      success: false,
      message: 'Failed to send 1-hour class reminders',
      error: error.message
    });
  }
}
