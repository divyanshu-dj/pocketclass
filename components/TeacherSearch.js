...
const TeacherSearch = ({ isShrunk, expandMenu }) => {
  ...
  return (
      <div className={`menu-search-bar relative z-50 transition duration-500 ${isShrunk ? 'translate-y-[-75px]' : '' }`}>
        <div onClick={() => isShrunk ? expandMenu() : null} className={`relative transition-all duration-500 ${isShrunk ? 'w-[250px] h-[48px]' : 'w-[650px]'}`} ref={dropdownRef}>
          ...
          <div ref={containerRef} className="flex items-stretch justify-start gap-1 relative h-full">
            ...
            <div className={`search-bar-option group ${isShrunk ? '!pl-6 !pr-3 max-w-[110px] font-medium' : 'max-w-[300px] !pl-8'}`}
            ...
            <div className={`search-bar-option group ${isShrunk ? '!px-3 max-w-[125px] text-center font-medium' : 'max-w-[350px]'}`}
            ...
            {/* Search Button */}
            <button
                className={`absolute right-2 -translate-y-1/2 top-1/2 flex items-center justify-center z-30 rounded-full  bg-red-500 hover:bg-red-600 text-white transition duration-500
                 ${isShrunk ? 'size-10 !right-1.5' : 'size-12'}`}
