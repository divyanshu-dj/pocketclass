...
const TeacherSearch = ({ isShrunk, expandMenu }) => {
  ...
  return (
      <div className={`menu-search-bar relative z-50 w-full transition-all duration-500 ${isShrunk ? 'max-w-[230px] h-[48px] translate-y-[-75px]' : 'max-w-[650px] max-md:px-3' }`}>
        <div onClick={() => isShrunk ? expandMenu() : null} className="relative" ref={dropdownRef}>
          ...
          <div ref={containerRef} className={`search-bar-wrapper transition-all`}>
            ...
            <div className={`search-bar-option group ${isShrunk ? '!pl-6 !pr-3 !w-auto font-medium' : '!pl-4 sm:!pl-8'}`}
            ...
            <div className={`search-bar-option group ${isShrunk ? '!pl-3 !pr-14 !w-auto text-center font-medium' : ''}`}
            ...
            <button
                className={`absolute right-2 -translate-y-1/2 top-1/2 flex items-center justify-center z-30 rounded-full  bg-red-500 hover:bg-red-600 text-white transition duration-500
                 ${isShrunk ? 'size-9 !right-1.5' : 'size-9 sm:size-12'}`}
