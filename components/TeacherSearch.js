        ...
        <div ref={containerRef} className="search-bar-wrapper transition-all">
          ...
          <div
            className={`search-bar-option group ${
              isShrunk ? '!pl-6 !pr-3 font-medium' : 
              `${isMenuSmall ? '!pl-5 !pr-4 md:!pl-8' : '!pl-5 !pr-4 md:!pl-8'}`
            }`}
            onClick={() => handleOptionClick('sub', 0)}
          >
            <div className={`search-hover initial bg-gray-200 ${
          ...
          {isShrunk ? (
            ...
            <div className={`search-options-separator ${
              !['picker', 'sub'].includes(activeDropdown) ? 'opacity-100' : 'opacity-0'
            }`}></div>
          )}
          ...
          <div
            className={`search-bar-option group ${
              isShrunk ? '!pl-3 !pr-14 !w-auto text-center font-medium' : 
              `${isMenuSmall ? '!px-4' : ''}`
            }`}
          ...
          >
            ...
            <ClearSearchIcon
              classes={`!right-16 ${
                dateRange[0] && dateRange[1] && activeDropdown === 'picker' ? 'opacity-100' : 'opacity-0'
              }`}
          ...
          <button
            className={`absolute right-1 md:right-2 -translate-y-1/2 top-1/2 flex items-center justify-center z-30 rounded-full bg-red-500 hover:bg-red-600 text-white transition duration-500 ${
              isShrunk ? 'size-9 !right-1.5' : 
              `${isMenuSmall ? 'size-11 right-1' : 'size-11 md:size-12'}`
            }`}
