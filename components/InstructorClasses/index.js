...
const InstructorClasses = ({ classes,bookings,reviews }) => {
    ...
    const groupedAppointments = bookings.reduce((acc, appointment) => {
        const existing = acc.find((group) => group.startTime === appointment.startTime);
        if (existing) {
            ...
            existing.payment.push({bookingId: appointment.id, paymentIntentId: appointment.paymentIntentId, studentId: appointment.student_id});
        } else {
            acc.push({
                ...appointment,
                ...
                payment: [{bookingId: appointment.id, paymentIntentId: appointment.paymentIntentId, studentId: appointment.student_id}],
            });
        }
        return acc;
    }, []);
            ...
            <div className='max-w-[1450px] mx-auto mb-4'>
                {selectedStatus==="Bookings" && bookings.length !== 0 ? (
                    groupedAppointments.map((appointment) => {
                        const classData = classes.find((c) => c.id === appointment.class_id);
                        return classData ? (
                            <InstructorCard
                                key={appointment.id}
