...
function InstructorCard({
  ...
  const handleMessageStudents = async () => {
    ...
    if (studentId.length === 1) {
      ...
      const chatRoom = await createGroupChatRoom();
      goToChat(classCreator, chatRoom?.id);
    }
  };
  ...
  const handleRefund = async () => {
    ...
    const id = appointmentId[0];
    console.log("Refunding class with ID:", id, typeof id);
    ...
    const userRef = doc(db, "Users", bookingData.student_id);
    ...
    const userEmails = [userData.email, ...bookingData.groupEmails];
    ...
    const endDateTime = moment
      ...
      .format("YYYY-MM-DDTHH:mm:ss");
    const refund = await fetch("/api/instructor-refund", {
      method: "POST",
    ...
    if (data.success) {
      ...
      console.log("Event deletion response:", eventData);
          ...
          <div className="flex flex-row flex-wrap justify-between flex-grow pl-5">
            <div className="flex flex-col flex-grow">
              <div className="flex flex-col flex-grow">
                ...
                {student_name && student_name.length > 0 && (
                  <div className="mt-2 flex items-center text-sm text-gray-500">
                    <svg
                      fill="#000000"
                      width="20px"
              ...
              {isBooking ? (
                <>
                  <button
                    ...
                    className="px-4  w-full py-1 border-solid border border-logo-red text-logo-red rounded-md hover:bg-logo-red hover:text-white hover:opacity-80 transition duration-200 ease-out"
                  >
                    Message Students
                  </button>
                  <button
                ...
                <>
                  ...
                  {/* Manage Schedule only if !Mindbody */}
                  {!isMindbody && (
                    <a
                      href={"/schedule/" + id}
                      className="px-4 text-center w-full py-1 border-solid border border-logo-red text-logo-red rounded-md hover:bg-logo-red hover:text-white hover:opacity-80 transition duration-200 ease-out"
