...
export default function MyGallery({ profileImage, coverImages = [] }) {
  ...
  // Show fallback if no image exists
  ...
  return (
    <div className="max-w-6xl mx-auto mt-10 px-4">
      ...
      <div className="grid dm2:grid-cols-3 grid-cols-1 gap-4">
        ...
        <div
          onClick={() => openModal(0)}
          className="sm:col-span-2 cursor-pointer group relative rounded-lg overflow-hidden"
        >
          <img
            src={effectiveProfileImage}
            alt="Profile"
            className="w-full h-full max-h-[400px] object-cover transition duration-300 group-hover:opacity-90 rounded-lg"
          />
        </div>
        ...
        {visibleImages.length > 0 && (
          <div
            className={`grid grid-cols-2 gap-2 ${
              visibleImages.length > 2 ? "grid-rows-2" : "grid-rows-1"
            }`}
          >
