...
function MusicianProfileCard1({ instructor }) {
  ...
  const fullName = `${instructor?.firstName || 'Unknown'} ${instructor?.lastName || ''}`.trim();
  const rating = "4.8"; // Static as not in DB
  const reviews = "124"; // Static as not in DB
  const price = "78"; // Static as not in DB
  const specialization = "Piano"; // Static as not in DB
        ...
        <div className="grow-0 shrink-0 basis-auto">
          ...
          <div className="flex justify-start items-center flex-row">
            ...
            <div className="flex justify-start items-center flex-row grow-0 shrink-0 basis-auto ml-[3px]">
              ...
              <p className="[font-family:'DM_Sans',sans-serif] text-base font-bold text-[#7d797a] grow-0 shrink-0 basis-auto ml-[3px] m-0 p-0">{specialization}</p>
            </div>
          </div>
          <div className="flex justify-start items-center flex-row mt-2">
            ...
            <div className="flex justify-start items-center flex-row grow-0 shrink-0 basis-auto ml-[3px]">
              <p className="[font-family:'DM_Sans',sans-serif] text-base font-bold text-[#261f22] grow-0 shrink-0 basis-auto m-0 p-0">{rating}</p>
              <p className="[font-family:'DM_Sans',sans-serif] text-base font-normal text-[#261f22] grow-0 shrink-0 basis-auto ml-1.5 m-0 p-0">({reviews} reviews)</p>
            </div>
          </div>
        </div>
        <div className="flex justify-start items-end flex-col grow-0 shrink-0 basis-auto">
          <p className="[font-family:'DM_Sans',sans-serif] text-2xl font-bold leading-6 text-[#261f22] grow-0 shrink-0 basis-auto m-0 pl-[21px] p-0">${price}</p>
